...
export const initSqlDb = async () => {
  ...
  if (isConnected) {
    ...
    // One-time data wipe for greenvalley tenant to resolve stale references and grade deletion errors
    try {
      const checkWipe = await sqlDb.query("SELECT COUNT(*) as cnt FROM audit_logs WHERE tenantId = 'greenvalley' AND action = 'System Reset'");
      if (checkWipe[0] && checkWipe[0].cnt > 0) {
        console.log('[SQL Init] Greenvalley database wipe has already been executed. Skipping.');
      } else {
        console.log('[SQL Init] Starting one-time remote database wipe for greenvalley tenant...');
        const tablesToWipe = [
          'student_enrollments',
          'parents',
          'addresses',
        ...
        for (const t of tablesToWipe) {
          try {
            await sqlDb.query(`DELETE FROM ${t} WHERE tenantId = ?`, ['greenvalley']);
          } catch (err) {
            console.warn(`[SQL Init WARNING] Failed to wipe table ${t}:`, err.message);
          }
        }
        await sqlDb.query("SET FOREIGN_KEY_CHECKS = 1");

        // Insert audit log to track that the reset occurred
        const logId = `log-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
        const timestamp = new Date().toISOString();
        await sqlDb.query(
          "INSERT INTO audit_logs (id, userId, userName, userRole, action, details, ipAddress, timestamp, tenantId) VALUES (?, 'System', 'System', 'System', 'System Reset', 'Greenvalley database wipe completed', '127.0.0.1', ?, 'greenvalley')",
          [logId, timestamp]
        );
        console.log('[SQL Init] One-time remote database wipe for greenvalley completed successfully.');
      }
    } catch (wipeErr) {
      console.error('[SQL Init ERROR] Failed to run database reset verification:', wipeErr.message);
    }

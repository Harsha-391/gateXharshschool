...
export default function StaffLeave({ showToast }) {
  ...
  const fetchPolicies = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const activeS = data.filter(p => p.employeeType === 'Staff' && p.status === 'Active');
        setActivePolicies(activeS);
        if (activeS.length > 0 && !form.leaveType) {
          setForm(prev => ({ ...prev, leaveType: activeS[0].leaveType }));
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const url = editingLeave ? `/api/staff-leaves/${editingLeave.id}` : '/api/staff-leaves';

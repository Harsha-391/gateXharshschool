...
export default function StaffLeave({ showToast, userProfile }) {
  ...
  const fetchPolicies = async () => {
    try {
      const res = await fetch(`/api/staff-leaves/policies?_t=${Date.now()}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleDocumentChange = (e) => {
    ...
    if (file) {
      // Store filename/mock path in attachment column
      ...
      <div className="glass-panel" style={{ padding: '24px', borderRadius: '20px', border: '1px solid var(--border-glass)', background: 'var(--bg-card)' }}>
        ...
        {activePolicies.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
            No active leave policies configured for Staff.
          </div>
        ) : (

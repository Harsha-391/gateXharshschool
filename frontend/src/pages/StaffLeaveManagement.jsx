...
export default function StaffLeaveManagement({ showToast }) {
  ...
  const fetchLeaves = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/staff-leaves/admin?${queryParams.toString()}`);
      if (res.ok) {
  ...
  const handleConfirmDecision = async () => {
    setActioning(true);
    try {
      const url = `/api/staff-leaves/${selectedLeave.id}/${decisionType}`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/staff-leaves/${id}/admin`, { method: 'DELETE' });
      if (res.ok) {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['ID', 'Staff Name', 'Employee ID', 'Leave Type', 'Title', 'From Date', 'To Date', 'Total Days', 'Status', 'Remarks', 'CreatedAt'];
    ...
    link.setAttribute("download", `staff_leaves_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="glass-panel" style={{
        ...
        <div>
          <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-main)' }}>Staff Leave Administration</h2>
      ...
      <div className="glass-panel" style={{
        ...
        <form onSubmit={handleSearchSubmit} style={{ display: 'flex', gap: '12px' }}>
          <div style={{ position: 'relative', flex: 1 }}>
            ...
            <input
              type="text"
              placeholder="Search by staff name, title, or leave type..."
              value={search}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            >
              ...
              <option value="CurrentlyOnLeave">Staff Currently On Leave</option>
            </select>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.88rem', textAlign: 'left' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-glass)', background: 'rgba(255,255,255,0.015)' }}>
                  <th style={{ padding: '16px', fontWeight: 700, color: 'var(--text-main)' }}>Staff Details</th>
                  ...
                  <tr key={leave.id} style={{ 
                    ...
                    <td style={{ padding: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        {leave.staffPhoto ? (
                          <img 
                            src={leave.staffPhoto} 
                            alt={leave.staffName} 
                            style={{ width: '36px', height: '36px', borderRadius: '50%', objectFit: 'cover', border: '1px solid var(--border-glass)' }}
                          />
                        ) : (
                          <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(16,185,129,0.1)', color: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
                            <User size={16} />
                          </div>
                        )}
                        <div>
                          <div style={{ fontWeight: 600, color: 'var(--text-main)' }}>{leave.staffName}</div>
                          <div style={{ fontSize: '0.74rem', color: 'var(--text-muted)' }}>ID: {leave.staffEmployeeId}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '16px' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.88rem', color: '#475569', lineHeight: 1.5 }}>
              Provide admin remarks for <strong>{selectedLeave?.staffName}</strong>'s request. These remarks will be visible in their employee portal.
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <textarea
                rows={3}
                placeholder="e.g. Leave approved as per request."
                value={remarks}
